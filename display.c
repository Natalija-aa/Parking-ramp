...
{ // ovo je inicijalizacija displeja na cetvorobitni rezim rada.
...
{ // funkcija koja brise sve sto pise na displeju
	LCD_RS = 0;
	LCD_EN = 1;
	LCD_D7 = 0;
	LCD_D6 = 0;
...
bit getbit(uint8_t n, uint8_t k)
{									   // funkcija koja vraca bit koji se nalazi na poziciji n
	uint8_t mask = 1 << k;	   // ova funkcija se koristi za ispisivanje karaktera na lcd
	uint8_t masked_n = n & mask; // za svaki karakter se cita svih 8 bita i salju se na lcd u funkciji writeChar
	bit thebit = masked_n >> k;
	return thebit;
}

void writeChar(uint8_t karakter)
{ // funkcija koja pise 1 karakter na displej- uzima bite iz karaktera i salje ih na displej
	LCD_RS = 1;
	LCD_EN = 1;
...
void newLine(void)
{ // funkcija za prelazak u novi red na lcdu
	LCD_RS = 0;
	LCD_EN = 1;
	LCD_D7 = 1;
	LCD_D6 = 1;
